...
const PARETO_CLASS_COLORS = [AUDI_GREEN, AUDI_YELLOW, '#7B8794'];
...
function LegacyChartTooltip({ active, label, payload }: RechartsTooltipProps) {
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-[#111111]/95 px-4 py-3 text-sm text-white shadow-2xl">
      ...
      <div className="mt-2 grid gap-1">
        {payload.map(item => (
          <span key={`${item.name}-${item.value}`} className="flex items-center justify-between gap-4 text-white/70">
            <span className="inline-flex items-center gap-2">
              <span className="h-2 w-2 rounded-full" style={{ backgroundColor: item.color ?? AUDI_RED }} />
              {item.name}
            </span>
            <strong className="text-white">
              {String(item.name).includes('%')
                ? percentTick(Array.isArray(item.value) ? item.value[1] : item.value ?? 0)
                : isClassCount
                  ? `${Number(item.value ?? 0).toLocaleString('es-BO')} productos`
                  : moneyTick(Array.isArray(item.value) ? item.value[1] : item.value ?? 0)}
            </strong>
          </span>
        ))}
      </div>
      {typeof paretoClass === 'string' && (
        <p className="mt-2 max-w-72 text-xs leading-5 text-white/50">
          Clase {String(paretoClass)} segun Pareto.
        </p>
      )}
    </div>
  );
...
export default function AdvancedAnalyticsScreen() {
  ...
  const loadBi = useCallback(async () => {
    ...
    setError(null);
    try {
      const [legacyDashboard, inventory, pareto, waterfall, heatmap] = await Promise.all([
        fetchAnalyticsDashboard({ idToken }),
  ...
  const legacyParetoChart = useMemo(
    () =>
      (dashboard?.pareto.items ?? []).slice(0, 10).map(item => ({
        ...item,
        shortName: truncateLabel(item.nombre, 18),
      })),
    [dashboard],
  );
        ...
        <section className="min-w-0 px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
          ...
          <section className="grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.85fr)]">
            <ChartPanel
              ...
              title="Productos que mueven la caja"
              aside={
                <span className="rounded-full border border-white/10 bg-[#111111] px-3 py-1 text-xs font-bold text-white/55">
                  Clase A activa
                </span>
              }
            >
              {legacyParetoChart.length ? (
                <div className="h-[390px] overflow-x-auto pb-2">
                  <div className="h-full min-w-[720px]">
                    <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0} debounce={80}>
                      <ComposedChart data={legacyParetoChart} margin={{ top: 12, right: 8, bottom: 8, left: 0 }}>
                        <CartesianGrid stroke={GRID} vertical={false} />
                        <XAxis dataKey="shortName" tick={{ fill: AXIS, fontSize: 12 }} axisLine={false} tickLine={false} />
                        <YAxis yAxisId="left" tickFormatter={moneyTick} tick={{ fill: AXIS, fontSize: 12 }} axisLine={false} tickLine={false} />
                        <YAxis yAxisId="right" orientation="right" tickFormatter={percentTick} tick={{ fill: AXIS, fontSize: 12 }} axisLine={false} tickLine={false} />
                        <Tooltip content={<LegacyChartTooltip />} />
                        <Legend wrapperStyle={{ color: AXIS, fontSize: 12 }} />
                        <Bar yAxisId="left" name="Ingresos" dataKey="totalCentavos" radius={[6, 6, 0, 0]} fill={AUDI_RED} />
            ...
            <ChartPanel
              ...
              title="Mix Pareto"
              aside={<TrendingUp className="h-5 w-5 text-audi-red" />}
            >
              <div>
                      ...
                      <PieChart>
                        <Pie data={paretoClassData} dataKey="value" nameKey="name" innerRadius={58} outerRadius={88} paddingAngle={5}>
                          {paretoClassData.map((entry, index) => (
                            <Cell key={entry.name} fill={PARETO_CLASS_COLORS[index % PARETO_CLASS_COLORS.length]} />
                          ))}
                        </Pie>
                        <Tooltip content={<LegacyChartTooltip />} />
                      </PieChart>
                ...
                <div className="mt-4 grid gap-3">
                  {(dashboard?.pareto.items ?? []).slice(0, 3).map(item => (
                    <div key={item.productoId} className="rounded-lg border border-white/10 bg-white/[0.035] p-3">
                      <div className="flex items-start justify-between gap-3">
                        <strong className="line-clamp-1 text-sm text-white">{item.nombre}</strong>
                        <span className="rounded bg-audi-red px-2 py-1 text-[11px] font-bold text-white">
          ...
          <section className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.9fr)]">
            <ChartPanel eyebrow="Tendencias" title="Ventas mensuales y margen">
              {trendChart.length ? (
                <div className="h-[340px] overflow-x-auto pb-2">
                  <div className="h-full min-w-[680px]">
                    <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0} debounce={80}>
                      <ComposedChart data={trendChart} margin={{ top: 12, right: 8, bottom: 8, left: 0 }}>
                        <CartesianGrid stroke={GRID} vertical={false} />
                        <XAxis dataKey="mes" tick={{ fill: AXIS, fontSize: 12 }} axisLine={false} tickLine={false} />
                        <YAxis tickFormatter={moneyTick} tick={{ fill: AXIS, fontSize: 12 }} axisLine={false} tickLine={false} />
                        <Tooltip content={<LegacyChartTooltip />} />
                        <Legend wrapperStyle={{ color: AXIS, fontSize: 12 }} />
                        <Bar name="Ingresos" dataKey="totalCentavos" radius={[6, 6, 0, 0]} fill={AUDI_RED} />
            ...
            <ChartPanel
              eyebrow="Estacionalidad"
              title="Audifonos"
              aside={<Boxes className="h-5 w-5 text-audi-red" />}
            >
              <div className="grid gap-3">
                {(dashboard?.tendencias.mesesFuertesAudifonos ?? []).map(month => (
                  <div key={month.mes} className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
                    <div className="flex items-center justify-between gap-4">
                      <strong className="text-white">{month.mes}</strong>
                      <span className="rounded bg-white px-3 py-1 text-xs font-bold text-[#111111]">
                        {month.cantidad} u.
                      </span>
                    </div>
                ...
                {!(dashboard?.tendencias.mesesFuertesAudifonos.length) && (
                  <p className="rounded-lg border border-white/10 bg-white/[0.035] p-4 text-sm font-semibold text-white/45">
          ...
          <section className="mt-5 grid gap-5 xl:grid-cols-2">
            <ChartPanel eyebrow="Investigacion de operaciones" title="Punto de pedido">
              <div>
                <p className="mb-4 text-sm font-semibold text-white/45">
                ...
                <div className="overflow-hidden rounded-lg border border-white/10">
                  {(dashboard?.inventario.reorderAlerts ?? []).slice(0, 8).map(item => (
                    <div key={item.productoId} className="grid gap-3 border-b border-white/10 bg-white/[0.025] p-4 last:border-b-0 md:grid-cols-[1fr_auto] md:items-center">
                      <div>
                        <strong className="block text-white">{item.nombre}</strong>
                      ...
                      <span className="rounded bg-audi-red px-3 py-1 text-xs font-bold text-white">
            ...
            <ChartPanel eyebrow="Liquidaciones" title="Stock muerto">
              <div>
                <p className="mb-4 text-sm font-semibold text-white/45">
          ...
          <div className="mb-4 mt-8 flex flex-col gap-2 border-t border-white/10 pt-6">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-audi-red">Modelos nuevos</p>
            <h2 className="text-2xl font-semibold text-white">Herramientas algorítmicas avanzadas</h2>
          </div>
